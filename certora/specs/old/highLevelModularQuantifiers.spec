 ...
 methods
{
    ...
    amountToShares(uint256 amount) returns (uint256) => symbolicAmountToSharesFunc(amount);       
    sharesToAmount(uint256 amount) returns (uint256) => symbolicSharesToAmountFunc(amount);    
    calcPremium(uint256 amount) returns (uint256) => symbolicPremium(amount);   
	...
	*/
	ghost symbolicPremium(uint256) returns uint256 {
		axiom  forall uint256 x. forall uint256 y. 
			( x < y => symbolicPremium(x) <= symbolicPremium(y) ) &&
	...
	*/
	ghost symbolicAmountToShares(uint256) returns uint256 {
		axiom  forall uint256 x. forall uint256 y. 
			( x < y => symbolicAmountToShares(x) <= symbolicAmountToShares(y) ) && 
			 symbolicAmountToShares(0) == 0 
		;
	}

	ghost symbolicSharesToAmount(uint256) returns uint256 {
		axiom  forall uint256 x. forall uint256 y. 
			( x < y => symbolicSharesToAmount(x) <= symbolicSharesToAmount(y) ) && 
			 symbolicSharesToAmount(0) == 0  
		;
	}
    ...
    */
	...
	}
	{
		...
		uint256 poolBalance1 = underlying.balanceOf(currentContract);
		uint256 supply1 = totalSupply();
        //require poolBalance1 == 2 * supply1;
