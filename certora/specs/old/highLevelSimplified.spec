 ...
 methods
{
    ...
    */
	...
	rule flashLoan_adds_value(address receiver, uint256 amount) {
		...
		require balance_pre == 2 * totalSupply_pre; // simplification - under approximation
	...
	rule user_solvency_on_flashLoan(address user) {
		...
		require poolBalance1 == supply1; // simplification - under approximation
	...
	}
	{
		...
		uint256 poolBalance1 = underlying.balanceOf(currentContract);
		uint256 supply1 = totalSupply();
        require poolBalance1 == 2 * supply1; // simplification - under approximation
